...
Immediately after the camera reads out, the images will be transferred via fiber networks to the LSST Data Facility (LDF; \S \ref{sec:LDF})\oldtext{ at the National Center for Supercomputing Applications (NCSA) in Illinois}.
...
\item \newtext{Any \texttt{DIASource} and \texttt{DIAForcedSource} records that exist, and difference image noise estimates where they do not, taken from the previous 12 months.} 
\oldtext{the previous 12 months of associated {\tt DIASource} records from the Prompt Products Database}
\item \oldtext{matching {\tt Object} IDs from the latest Data Release, if they exist, and 12 months of their {\tt DIASource} records}
