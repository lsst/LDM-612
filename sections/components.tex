...
\oldtext{The LSST Data Facility (LDF) provides the primary site for LSST data processing and data access.}
\newtext{The LSST Data Facility (LDF) together with the CC-IN2P3 in France, where 50$\%$ of the  Data Release Processing (DRP) will be done, provide the primary sites for LSST data processing.}  
\citeds{LDM-230} describes the concept of operations for the LDF.
The LDF hosts a variety of computational services, including the Prompt Processing services that run the Alert Production pipelines (\S \ref{sec:AP}) and the Batch Production services that run the annual Data Release Processing (\S \ref{sec:drp}). \newtext{All Prompt Processing and Alert Production is run at NCSA. } 
...
The basic modules include single-frame image processing (e.g., instrument signature removal, photometric and astrometric calibration), difference imaging analysis (DIA; the subtraction of the new visit image \oldtext{and} \newtext{from} a coadded template image constructed from the \oldtext{last} \newtext{most recent} Data Release), and source detection, association, and measurement \citedsp{LSE-163}.
...
Due to the need for Data-Release-derived templates, Alert Production cannot run at full scale and full fidelity during commissioning \oldtext{and} \newtext{nor} the first year of operations.  \newtext{LSST DM is currently investigating options for Alert Production in year one.}
...
While the \texttt{SSObject} catalog will associate past \texttt{DIAsources} with a new \texttt{SSObject} as far back \newtext{as}  is practical, revised alerts will not be issued.
...
\marginpar{I am concerned that readers might interpret the following paragraph as us setting an upper limit on the number of brokers ... so I suggest a rephrasing.}
\oldtext{Due to finite bandwidth out of the LDF, only selected community brokers will receive the full alert stream. Science users may access LSST alerts through a community broker, or through the mini-broker if they have data access rights (\S \ref{sec:data_rights}). An allocation of 10 Gbps is baselined for alert stream transfer from the LDF. With an estimated packet size of 82 KB and 10,000 alerts per visit, about 7 brokers can receive the full stream if 5 seconds of the 60 second latency budget is devoted to outbound data transfer. The selection process described in \S \ref{sec:community_brokers} is most appropriate for this number of streams. If only one or two full streams could be supported
}

\newtext{Due to the finite bandwidth out of the LDF, only a limited number of selected community brokers will receive the full alert stream.
It will not be possible for individual science users to subscribe directly to the full LSST alert stream, rather, science users may access LSST alerts through a community broker, or through the mini-broker if they have data access rights (\S \ref{sec:data_rights}).
An allocation of 10\,Gbps is baselined for alert stream transfer from the LDF, with an estimated packet size of 82\,KB and 10,000 alerts per visit.
Based on these numbers, up to 7 brokers could receive the full stream if 5\,seconds of the 60\,second latency budget is devoted to outbound data transfer.
Although the selection process described in \S \ref{sec:community_brokers} is most appropriate for this number of streams, the eventual number of selected community brokers (\S \ref{sec:numbrokers}) depends on several quantities such as packet size, number of alerts per visit, rate of alert distribution , and is still under discussion. 
...
\oldtext{updated \texttt{DIASource} lightcurve} \newtext{\texttt{DIASource}-updated lightcurve.}
