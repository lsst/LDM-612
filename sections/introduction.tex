...
An LSST science user will be able to use the contents of a single alert packet \oldtext{and} \newtext{to} make a \oldtext{rapid but informed decision} \newtext{decision that is both rapid and informed}  about whether the event is relevant to their scientific goals.
LSST expects to produce about ten million of these alerts nightly\footnote{Transients in the alert stream are required to be 90\% complete and 95\% pure at 6$\sigma$ \citeds{LSE-30}, so less than 5\% of the alert stream will consist of false positives.}.
...
\marginpar{'it' here to me implies the 'alert stream' - but  users will not add to the alert stream itself, rather,  users will use the contents of the alerts, made accessible by community brokers to derive new additional information to inform follow-up}
\oldtext{To do so, astronomers will rely on third party \textit{community brokers}, software systems that receive the full LSST alert stream and add additional information to it.} 
\newtext{To do so, astronomers will rely on third party \textit{community brokers}, software systems that receive the full LSST alert stream, to derive additional information to refine the subset of events of interest and better inform follow-up campaigns.}
...
The large volume of the alert stream and the finite bandwidth from the LSST Data Facility \oldtext{necessitates} \newtext{necessitate} a proposal process to select community brokers to receive the full stream.
